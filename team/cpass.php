...
if(isset($_POST['submit']))
{
    ...
    $folder = "assets/profile/".$img;
    move_uploaded_file($tmpname,$folder);

    if(empty($userid)) {
        $_SESSION['ErrorMessage'] = "All field must be fill out";
        Redirect_To('cpass.php');
    }

    if (empty($password)) {
        $_SESSION['ErrorMessage'] = "Password must needed";
        Redirect_To('cpass.php');
    }

    if (!empty($password) AND !empty($userid))
    {
        $check = "SELECT * FROM `tbl_team` WHERE USER_ID = '$userid'";
        $data = mysqli_query($conn, $check);
        $row = mysqli_num_rows($data);

        if ($row == 1) 
        {
          $_SESSION['ErrorMessage'] = "User id is aleady taken";
          Redirect_To('cpass.php');
        }
        else
        {
          $ins = "UPDATE `tbl_team` SET `PROFILE`='$folder',USER_ID='$userid',`PASSWORD`='$password' WHERE `ID` = '$tid'";
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          ...
          <div class="body-wrapper">
            <h3>Change Data</h3><hr>
            <br>
            <form action="cpass.php" method="POST" enctype="multipart/form-data">
              <div>
                <?php 
                    echo Message();
                    echo SuccessMessage();
              ...
              <button type="submit" name="submit" class="btn btn-block btn-primary">Change Password</button>
            </form>
