  ...
  if (isset($_POST['submit'])) 
{
    ...
    $file = $_FILES['file_vids'];

    $fileName = $_FILES['file_vids']['name'];
    $fileTmpName = $_FILES['file_vids']['tmp_name'];
    $fileSize = $_FILES['file_vids']['size'];
    $fileError = $_FILES['file_vids']['error'];
    $fileType = $_FILES['file_vids']['type'];
    ...
    $allowed = array('mp4', 'avi', 'mkv');
        ...
        if(empty($title)) 
        {
            ...
            Redirect_To('add_lessons.php');
        }
        if(empty($course)) 
        {
            $_SESSION['ErrorMessage'] = "Select a playlist.";
            Redirect_To('add_lessons.php');
        }
        ...
        if (!empty($title)) 
        {
            $insert = "INSERT INTO `tbl_lesson`(`COURSE_ID`, `COMP_ID`, `EMP_ID`, `EP_NAME`, `VIDEO`, `LOGS`, `STATUS`) VALUES ('$course','$c','$e','$title','$fileDes',now(),1)";
            ...
            {
                $_SESSION['ErrorMessage'] = "Something worng.";
                Redirect_To('add_lessons.php');
            }
        }
}
      ...
      <div class="main-panel">
        <div class="content-wrapper">
          ...
          <div class="body-wrapper">
            ...
            <div class="content">
                <h3>ADD CHAPTER'S</h3>
                <p>Upload the lessons of the course.</p><hr>

                <form action="add_lessons.php" method="POST" enctype="multipart/form-data" class="formWrapper">
                    <div>
                        <?php 
                            echo Message();
                            echo SuccessMessage();
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name='course'>
                            ...
                            <?php
                                $sql = "SELECT * FROM `tbl_course` WHERE COMP_ID = '$c' AND EMP_ID = '$e'";
                                $da = mysqli_query($conn,$sql);
                                while ($rows = mysqli_fetch_assoc($da)) 
                                {
                            ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" class="form-control" name="file_vids">
                    </div>
                    <div class="form-group">
                        <button class="btn btn-block btn-success" name="submit">Upload</button>
                    </div>
                </form>
