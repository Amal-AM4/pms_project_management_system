          ...
          <div class="body-wrapper">
            ...
            <div class="body-wrapper-right">
              <div class="right-container">
                <?php
                  $sel_team = "SELECT NAME,PROFILE,COMP_ID,EMAIL,DEPT FROM `tbl_team` WHERE ID = '$p_two'";
                  $sel_exe = mysqli_query($conn, $sel_team);
                  $row = mysqli_fetch_assoc($sel_exe);

                  $comID = $row['COMP_ID'];

                  $sel_com = "SELECT COMP_NAME,PHONE FROM `tbl_companies` WHERE ID = '$comID'";
                  $selc_exe = mysqli_query($conn, $sel_com);
                  $rowz = mysqli_fetch_assoc($selc_exe);
                ?>
                <img src="../team/<?php echo $row['PROFILE'];?>">
                <h3><?php echo $row['NAME'];?></h3>
                <small><?php echo $row['DEPT'];?></small>
                <br>
                <p>Company Name: <?php echo $rowz['COMP_NAME'];?></p>
                <p>Email: <?php echo $row['EMAIL'];?></p>
                <p>Contact No: <?php echo $rowz['PHONE'];?></p>
              </div>
            </div>
          </div>
